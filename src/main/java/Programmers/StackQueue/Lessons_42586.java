 ...
 */
public class Lessons_42586 {
    public int[] solution(int[] progresses, int[] speeds) {
        ArrayList<Integer> tempAnswer = new ArrayList<>();
        int program_num = progresses.length;
        int deploy_num = 0;
        while(deploy_num < program_num){
            int today_deploy = 0;
            for(int i=deploy_num; i<program_num; i++){
            ...
            if(today_deploy>0){
                tempAnswer.add(today_deploy);
            }
        }

        int[] answer = new int[tempAnswer.size()];
        for(int i = 0; i<tempAnswer.size();i ++) {
            answer[i] = tempAnswer.get(i).intValue();
        }
        return answer;
    }

    public static void main(String args[]) {
        Lessons_42586 lessons_42586 = new Lessons_42586();
        int[] test_progresses = {93, 30, 55};
        int[] test_speeds = {1, 30, 5};
        int[] answer = {2, 1};
        int[] result = lessons_42586.solution(test_progresses, test_speeds);
