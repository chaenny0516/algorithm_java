 ...
 */
public class Lessons_2906 {

    public int[] solution(int []arr) {
        ArrayList<Integer> tempAnswer = new ArrayList<>();
        tempAnswer.add(arr[0]);
        for(int i=1; i< arr.length; i++){
            int recentValue = tempAnswer.get(tempAnswer.size()-1); // 인덱스의 마지막 값
            if(recentValue != arr[i]){
    ...
    public static void main(String[] args) {
        Lessons_2906 lessons_2906 = new Lessons_2906();
        int[] testcase1 = {1, 1, 3, 3, 0, 1, 1};
        int[] answer1 = {1, 3, 0, 1};
        int[] result1 = lessons_2906.solution(testcase1);
